...
responses=(
    ...
    ["ai_get_macaddress.json"]="{$BASE_URL}/ai?command=getMacAddress"
    ["ai_get_modeldescription.json"]="{$BASE_URL}/ai?command=getModelDescription"
...
for filename in "${!responses[@]}"; do
    ...
    curl -s -o "./$DEVICE_ID/$filename" "$url"
...
sed -i -E "s/[0-9a-fA-F:]+/$MAC_ADDRESS/g" "$DEVICE_ID/ai_get_macaddress.json"
