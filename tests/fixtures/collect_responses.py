...
class record_call:
    def __init__(
        self,
        resulting_filename: str,
        relative_url: str,
        category_special: bool,
        de_identify_mac: bool,
    ):
...
def collect_responses(device_id):
    ...
    record_calls = {
        record_call(
            "ai_get_devicestatus.json", "/ai?command=getDeviceStatus", False, False
        ),
        record_call("ai_get_fwversion.json", "/ai?command=getFWVersion", False, False),
        record_call(
            "ai_get_lastpushnotifications.json",
            "/ai?command=getLastPUSHNotifications",
            False,
            False,
        ),
        record_call(
            "ai_get_macaddress.txt",
            "/ai?command=getMacAddress",
            category_special=False,
            de_identify_mac=True,
        ),
        record_call(
            "ai_get_modeldescription.txt",
            "/ai?command=getModelDescription",
            False,
            False,
        ),
        record_call(
            "ai_get_updatestatus.json", "/ai?command=getUpdateStatus", False, False
        ),
        record_call(
            "NA",
            "/hh?command=getCategories",
            category_special=True,
            de_identify_mac=False,
        ),
    ...
    for current_call in record_calls:
        ...
        if current_call.category_special:
            ...
            logging.error(
                f"Error: Unrecognized file type for {output_file}. Skipped collecting response."
            )


def collect_categories_details(
    filename_categories: str,
    filename_commands: str,
    base_url: str,
    ...
    for curr_category_id in categories_json:
        ...
        for curr_command in commands:
            ...
            all_command_details.append(
                command_details(curr_command, command_details_json)
            )

    categories_data = [
        {"id": c.id, "category": c.category, "commands": c.commands} for c in categories
    ]
...
def select_device() -> str:
    ...
    print(
        "- When 0 is selected and the device already exists, it will be completely overwritten."
    )
...
def main():
    ...
    if args.device_id and shared.is_valid_device(args.device_id):
        ...
        print(
            "No port or device_id provided or device config not found. Starting interactive mode."
        )
