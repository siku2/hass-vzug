...
async def assert_ai_get_device_status_core(vzug_client, expected_result):
    ...
    assert (
        device_status["ProgramEnd"]["End"]
        == expected_result.ai_device_status["ProgramEnd"]["End"]
    )
    assert (
        device_status["ProgramEnd"]["EndType"]
        == expected_result.ai_device_status["ProgramEnd"]["EndType"]
    )
...
async def assert_ai_get_last_push_notifications(vzug_client, expected_result):
    ...
    for i in range(len(expected_result.ai_last_push_notifications)):
        assert (
            last_push_notifications[i]["date"]
            == expected_result.ai_last_push_notifications[i]["date"]
        )
        assert (
            last_push_notifications[i]["message"]
            == expected_result.ai_last_push_notifications[i]["message"]
        )
...
async def assert_ai_get_model_description(vzug_client, expected_result):
    ...
    assert (
        model_description.replace(" Emulator", "")
        == expected_result.ai_model_description
    )
...
async def assert_ai_get_update_status(vzug_client, expected_result):
    ...
    assert (
        update_status["isAIUpdateAvailable"]
        == expected_result.ai_update_status["isAIUpdateAvailable"]
    )
    assert (
        update_status["isHHGUpdateAvailable"]
        == expected_result.ai_update_status["isHHGUpdateAvailable"]
    )
    ...
    for i in range(len(expected_result.ai_update_status["components"])):
        assert (
            update_status["components"][i]["name"]
            == expected_result.ai_update_status["components"][i]["name"]
        )
        assert (
            update_status["components"][i]["running"]
            == expected_result.ai_update_status["components"][i]["running"]
        )
        assert (
            update_status["components"][i]["available"]
            == expected_result.ai_update_status["components"][i]["available"]
        )
        assert (
            update_status["components"][i]["required"]
            == expected_result.ai_update_status["components"][i]["required"]
        )
        assert (
            update_status["components"][i]["progress"]["download"]
            == expected_result.ai_update_status["components"][i]["progress"]["download"]
        )
        assert (
            update_status["components"][i]["progress"]["installation"]
            == expected_result.ai_update_status["components"][i]["progress"][
                "installation"
            ]
        )
...
async def assert_aggregate_meta(vzug_client, expected_result):
    meta = await vzug_client.aggregate_meta()

    assert meta.mac_address == expected_result.aggregate_meta.mac_address
    assert meta.model_id == expected_result.aggregate_meta.model_id
    assert meta.model_name == expected_result.aggregate_meta.model_name
    assert meta.device_name == expected_result.aggregate_meta.device_name
    assert meta.serial_number == expected_result.aggregate_meta.serial_number
